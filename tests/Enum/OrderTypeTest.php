<?php

namespace OrderCoreBundle\Tests\Enum;

use OrderCoreBundle\Enum\OrderType;
use PHPUnit\Framework\Attributes\CoversClass;
use Tourze\PHPUnitEnum\AbstractEnumTestCase;

/**
 * @internal
 */
#[CoversClass(OrderType::class)]
class OrderTypeTest extends AbstractEnumTestCase
{
    public function testToArrayMethod(): void
    {
        $result = OrderType::DEFAULT->toArray();

        $this->assertIsArray($result);
        $this->assertArrayHasKey('value', $result);
        $this->assertArrayHasKey('label', $result);
        $this->assertSame('default', $result['value']);
        $this->assertSame('实物销售', $result['label']);
    }

    public function testGenOptionsMethod(): void
    {
        $options = OrderType::genOptions();

        $this->assertIsArray($options);
        $this->assertCount(2, $options);

        foreach ($options as $option) {
            $this->assertArrayHasKey('value', $option);
            $this->assertArrayHasKey('label', $option);
            $this->assertIsString($option['value']);
            $this->assertIsString($option['label']);
        }

        $values = array_column($options, 'value');
        $this->assertContains('default', $values);
        $this->assertContains('store-service', $values);
    }
}
